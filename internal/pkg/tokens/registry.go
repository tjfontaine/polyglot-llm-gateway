...
type Registry struct {
	...
	provider ports.Provider // Optional provider to check for TokenCountProvider interface
}
...
func (r *Registry) SetProvider(provider ports.Provider) {
	r.provider = provider
}
...
func (r *Registry) CountTokens(ctx context.Context, req *domain.TokenCountRequest) (*domain.TokenCountResponse, error) {
	...
	if r.provider != nil {
		if tcp, ok := r.provider.(ports.TokenCountProvider); ok {
