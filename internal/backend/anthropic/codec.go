// Codec provides conversion between Anthropic API format and canonical format.
package anthropic
...
// NewCodec creates a new Anthropic codec.
func NewCodec() *Codec {
	return &Codec{}
}
...
func APIRequestToCanonical(apiReq *MessagesRequest) (*domain.CanonicalRequest, error) {
...
func collapseContentBlocks(blocks ContentBlock) (string, error) {
	if len(blocks) == 0 {
...
func CanonicalToAPIRequest(req *domain.CanonicalRequest) *MessagesRequest {
...
func CanonicalToAPIRequestWithImageFetching(ctx context.Context, req *domain.CanonicalRequest, fetcher *codec.ImageFetcher) (*MessagesRequest, error) {
	var fetchErr error
...
func canonicalToAPIRequestInternal(req *domain.CanonicalRequest, fetcher *codec.ImageFetcher, fetchErr *error) *MessagesRequest {
	var systemBlocks SystemMessages
	...
	for _, m := range req.Messages {
		...
		case "assistant":
			...
			if len(m.ToolCalls) > 0 {
				...
				if m.Content != "" {
					content = append(content, ContentPart{Type: "text", Text: m.Content})
				}
	...
	if req.ToolChoice != nil {
		...
		case map[string]interface{}:
			if name, ok := tc["function"].(map[string]interface{})["name"].(string); ok {
				apiReq.ToolChoice = &ToolChoice{Type: "tool", Name: name}
			}
		}
	}
...
func APIResponseToCanonical(apiResp *MessagesResponse) *domain.CanonicalResponse {
...
func APIResponseToCanonicalWithRateLimits(apiResp *MessagesResponse, rateLimits *RateLimitHeaders) *domain.CanonicalResponse {
	content := ""
...
func CanonicalToAPIResponse(resp *domain.CanonicalResponse) *MessagesResponse {
	var contentBlocks []ResponseContent
	finishReason := ""
	...
	if len(contentBlocks) == 0 {
		contentBlocks = []ResponseContent{{Type: "text", Text: ""}}
	}
...
func CanonicalToAPIChunk(event *domain.CanonicalEvent) *ContentBlockDeltaEvent {
...
func convertRichContentToAnthropic(content *domain.MessageContent, fetcher *codec.ImageFetcher, fetchErr *error) ContentBlock {
