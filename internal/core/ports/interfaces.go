...
type Provider interface {
	...
	Complete(ctx context.Context, req *domain.CanonicalRequest) (*domain.CanonicalResponse, error)

	// Stream returns a channel of events.
	// The channel MUST be closed by the provider when done.
	Stream(ctx context.Context, req *domain.CanonicalRequest) (<-chan domain.CanonicalEvent, error)

	// ListModels returns the models supported by the provider/frontdoor pairing.
	ListModels(ctx context.Context) (*domain.ModelList, error)
}
...
type PassthroughProvider interface {
	...
	SupportsPassthrough(sourceType domain.APIType) bool
	...
	CompleteRaw(ctx context.Context, req *domain.CanonicalRequest) ([]byte, error)
	...
	StreamRaw(ctx context.Context, req *domain.CanonicalRequest) (<-chan domain.CanonicalEvent, error)
}
...
type ResponsesProvider interface {
	...
	CreateResponse(ctx context.Context, req *domain.ResponsesAPIRequest) (*domain.ResponsesAPIResponse, error)

	// StreamResponse handles a streaming Responses API request.
	StreamResponse(ctx context.Context, req *domain.ResponsesAPIRequest) (<-chan domain.CanonicalEvent, error)

	// GetResponse retrieves a stored response by ID.
	GetResponse(ctx context.Context, id string) (*domain.ResponsesAPIResponse, error)

	// CancelResponse cancels an in-progress response.
	CancelResponse(ctx context.Context, id string) (*domain.ResponsesAPIResponse, error)
}
...
type TokenCountProvider interface {
	...
	CountTokensCanonical(ctx context.Context, req *domain.TokenCountRequest) (*domain.TokenCountResponse, error)
